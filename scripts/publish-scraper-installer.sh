...
VERSION="$(node -p "JSON.parse(require('fs').readFileSync('terminal/scraper-release.json','utf8')).version")"
FILENAME="$(node -p "JSON.parse(require('fs').readFileSync('terminal/scraper-release.json','utf8')).filename")"
...
echo "==> Push main (terminal download button + scraper-release.json)"
...
if gh release view "$TAG" >/dev/null 2>&1; then
  ...
  gh release create "$TAG" "$INSTALLER" \
    ...
    --notes "Windows NSIS installer for OddsLocker Scraper ${VERSION}."
fi
...
if [[ -n "$RAILWAY_CMD" ]]; then
  ...
  echo "Railway CLI not logged in. Run from terminal/ service directory:"
  echo "  npx @railway/cli login"
  echo "  npx @railway/cli link"
  echo "  npx @railway/cli variables set SCRAPER_INSTALLER_URL=${DOWNLOAD_URL}"
fi

echo ""
echo "Done. Open your terminal URL — header should show Download OddsLocker Scraper v${VERSION}."
echo "Verify: curl -s https://YOUR-HOST/health | grep scraperDownload"
